...
from helpers.log import log_general, LogLevel
...
def load_config() -> Config:
    def _check_single_field(config: Dict[str, str], key: str) -> bool:
        if not key in config:
            log_general(LogLevel.ERROR, f"Config file does not contain required key {key}")
            return False

        if config[key].strip() == key.upper():
            log_general(LogLevel.ERROR, f"Config entry {key} has default value {key.upper()}")
            return False

        if config[key].strip() == "":
            log_general(LogLevel.ERROR, f"Config entry {key} is empty")

        return True

    config_sample_result: LoadJsonResult = load_json(Path.cwd() / _CONFIG_SAMPLE_FILE)
    if not config_sample_result.success:
        log_general(LogLevel.INFO, "Config sample file does not exist, creating it")
        result: SaveJsonResult = save_json(Path.cwd() / _CONFIG_SAMPLE_FILE, _gen_default_config())
        if not result.success:
            log_general(LogLevel.CRITICAL, "Failed to create a config sample file")
            raise Exception("Failed to create a config sample file")

    config_result: LoadJsonResult = load_json(Path.cwd() / _CONFIG_FILE)
    if not config_result.success:
        log_general(LogLevel.CRITICAL, "Failed to load the config file")
    ...
    if not valid_data:
        log_general(LogLevel.CRITICAL, "Invalid data in config file")
