...
def test_handle_commands_reserved_sync():
    handle_commands_path = Path("src/bot/chat/handle_commands.py")
    assert handle_commands_path.exists()
    
    with open(handle_commands_path, "r") as f:
        tree = ast.parse(f.read())
        
    found_builtin_commands = set()
                    ...
                    if first_pattern.__class__.__name__ == "MatchValue":
                        ...
                        if isinstance(value_node, ast.Constant) and isinstance(value_node.value, str):
                            if value_node.value.startswith("!"):
                                cmd_name = value_node.value.lstrip("!")
    ...
    for cmd in found_builtin_commands:
        assert cmd in RESERVED_NAMES, f"Command '!{cmd}' found in handle_commands.py but not in RESERVED_NAMES in src/bot/core/reserved_names.py"
