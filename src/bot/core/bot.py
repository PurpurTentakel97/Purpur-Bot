...
def get_bot(bot_id: int) -> Optional[BotConfigDB]:
    return select_bot_db(bot_id)


def get_bots_by_twitch_id(twitch_id: str) -> list[BotConfigDB]:
    return select_bots_by_twitch_id_db(twitch_id)


def add_bot(twitch_id: str) -> Optional[int]:
    return insert_bot_db(twitch_id)


def update_bot_name(bot_id: int, name: str) -> bool:
