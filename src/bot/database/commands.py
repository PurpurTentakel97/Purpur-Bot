...
def select_command_by_id(command_id: int) -> Result[BasicCommandDB]:
    return PROGRAMM_PARTS.database.select_one(table_name=TABLE_NAME, where={"id": command_id}, type_=BasicCommandDB)
...
def update_command_by_id(command_id: int, data: dict[str, Any]) -> Result[BasicCommandDB]:
    result = PROGRAMM_PARTS.database.update(table_name=TABLE_NAME, where={"id": command_id}, data=data)
...
def delete_command_by_id(command_id: int) -> Result[None]:
    return PROGRAMM_PARTS.database.delete(table_name=TABLE_NAME, where={"id": command_id})
