...
from bot.database.types.fields import FIELD_DISCORD_SERVER_NAME
...
def select_discord_servers_by(where: dict[str, Any]) -> Result[list[DiscordServerDB]]:
    return PROGRAMM_PARTS.database.select_all(table_name=TABLE_DISCORD_NAME, where=where, type_=DiscordServerDB)


def select_discord_by(where: dict[str, Any]) -> Result[DiscordServerDB]:
    return PROGRAMM_PARTS.database.select_one(table_name=TABLE_DISCORD_NAME, where=where, type_=DiscordServerDB)


def insert_discord_server(bot_id: int, server_id: int, server_name: str) -> Result[int]:
    return PROGRAMM_PARTS.database.insert(
        table_name=TABLE_DISCORD_NAME,
        data={FIELD_BOT_ID: bot_id, FIELD_DISCORD_SERVER_ID: server_id, FIELD_DISCORD_SERVER_NAME: server_name},
    )


def update_discord_server_by_id(id_: int, data: dict[str, Any]) -> Result[None]:
    return PROGRAMM_PARTS.database.update(table_name=TABLE_DISCORD_NAME, where={FIELD_ID: id_}, data=data)


def delete_discord_server(bot_id: int, server_id: int) -> Result[None]:
    return PROGRAMM_PARTS.database.delete(
        table_name=TABLE_DISCORD_NAME, where={FIELD_BOT_ID: bot_id, FIELD_DISCORD_SERVER_ID: server_id}
    )
